...
plot_samp_dist(offset=0)
```

```{r}
d <- density(samp_dist_vals, bw=0.02)
d_x <- d$x
d_y <- d$y * length(samp_dist_vals) * samp_bin_width
```
...
```{r}
p <- (ggplot(samp_dist, aes(sample_mean, fill='red'))
 + geom_histogram(breaks = full_samp_breaks)
 + geom_density(aes(y = after_stat(count * samp_bin_width)),
                bw = 0.02, alpha = 0.3))
p
```
...
sum(ggp_build$data[[1]]$y)
```

```{r}
kde_data <- ggplot_build(p)$data[[2]]
plot(kde_data$x, kde_data$y)
```
