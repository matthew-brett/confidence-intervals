...
Here is a *population* of values.

```{r}
df <- read.csv('data/birth_weights.csv')
birth_weights <- df[['BWEIGHT']]
birth_weights
```
...
But - we don't know anything about this population yet.  All we have is a single random sample of 50 values.

```{r}
bw_sample <- sample(birth_weights, size=50)
bw_sample
```
...
another_sample <- sample(birth_weights, size=50)
mean(another_sample)
```
...
for (i in 1:10000) {
    this_sample <- sample(birth_weights, size=50)
...
```{r}
# Draw another random sample.  Look at the mean.
yet_another_sample <- sample(birth_weights, size=50)
...
Just so you can see I am not crazy, here is the confidence interval calculated in a slightly different way (using some mathematical assumptions).  It gives a very similar result.

```{r}
# Calculate the confidence intervals using normal distribution assumptions.
confint(lm(bw_sample ~ 1))
```
...
boot.ci(boot(bw_sample, meanfun, 10000), type = 'basic')
```
